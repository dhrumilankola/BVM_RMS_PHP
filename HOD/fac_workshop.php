...
if(isset($_GET["workshopid"])){
   ...
   $aid=$rows['aid'];			 
	$photos=$rows['photos'];
					...
					$timestamp = strtotime($rows['sdate']);
							$sdate = date("d-m-Y",$timestamp );
							
							
					$timestamp = strtotime($rows['edate']);
              ...
              else
                {
                    ...
                    header("location: fac_patent.php");
                    exit();
                 }
     }
      else
...
<head>
  ...
  <title>BRMS | HOD View Employee Details</title>
                          ...
                          <div class="row">
                            ...
                            <tbody>
                                    <tr>
                                      ...
                                      //echo "<img src = '../Faculty/".$row_did['department']."/".$row_user['uname']."/".date('Y')."/Projects/Documents/". $rows['upload'] . "'class='img-responsive' style=width:100px;>";
                                        	?>
											<?php 
                                         // echo $rows['photos']."&nbsp&nbsp&nbsp";

                                           if($row_user['user_type'] == 'Faculty')
                                             { 
                                               echo "<img src = '../Faculty/".$row_did['department']."/".$row_user['uname']."/".date('Y')."/Workshop/Photos/". $rows['photos'] . "'class='img-responsive' style=width:100px;>";
											   //echo "<a class='btn btn-info btn-sm' href='../Faculty/".$row_did['department']."/".$row_user['uname']."/".date('Y')."/Workshop/Photos/".$rows['photos']."' download='".$rows['photos']."'><i class='fa fa-download'></i></a>";
                                          }
                                            ...
                                            <td><?php echo "<a class='btn btn-info btn-sm' href='../Faculty/".$row_did['department']."/".$row_user['uname']."/".date('Y')."/Workshop/Photos/".$rows['photos']."' download='".$rows['photos']."'><i class='fa fa-download'></i></a>"; 	
											?>
                                          </td>  
                                      </tr>
                            </tbody>

...
//if(isset($_FILES['profile'])){
  ...
  //foreach($_FILES['profile']['tmp_name'] as $key => $tmp_name ){
    //$file_name = $key.$_FILES['profile']['name'][$key];
    //$file_tmp =$_FILES['profile']['tmp_name'][$key];
...
//             }
 ...
 //$desired_dir= $rowdid['department']."/". $row['uname']."/".'/Images';       
	   ...
	   //if(empty($errors)==true){
           /*if(is_dir($desired_dir)==false){
            ...
            if(is_dir("$desired_dir/".$file_name)==false){
                move_uploaded_file($file_tmp,$rowdid['department']."/". $row['uname']."/".'/Images/'.$file_name);
            }else{                  //rename the file if another one exist
                $new_dir=$rowdid['department']."/". $row['uname']."/".'/Images/'.$file_name.time();
...
// Profile Pic//
  ...
  if(in_array($rfileActualExt, $rallowed)) {
     ...
     $sql= "UPDATE user SET uname='$uname',mno='$mno' ,gender='$gender',  
	 email='$email',   city='$city',  bday='$bday' , addr='$addr',  zip='$zip',
	 desig='$desig',  schlspec='$schlspec',  school='$school', yschool='$yschool',
	 rschool='$rschool',   hspec='$hspec',   hschool='$hschool',  hyear='$hyear', 
	 hresult='$hresult', dspec='$dspec', sdiploma='$sdiploma',ydiploma='$ydiploma',
	 rdiploma='$rdiploma',degreespec='$degreespec',degree='$degree',ydegree='$ydegree',
	 rdegree='$rdegree',   pgspec='$pgspec',  pg='$pg',  ypg='$ypg',   rpg='$rpg',
	 otherspec='$otherspec',sothers='$sothers',yothers='$yothers',rothers='$rothers',
	 profile='$profile',   resume='$resume'  WHERE aid='$session_aid'";
      ...
      <div class="container-fluid">
			...
			if(elem.value.match(numericExpression)){
				...
				return false;
                               ...
                               <div class="form-group">
                                  ...
                                  <input type="text" class="form-control" placeholder="Enter ..." name="mno" id='mob' value="<?php echo $mno; ?>" onchange='return formMob()'>
                               </div>
                  </div>
                  <div class="col-sm-3">
				  <script type='text/javascript'>
		...
		function formGen(){
			...
			var gen = document.getElementById('gen');
				...
				if(madeSelection(gen, "Please Select a Gender")){
		...
		function madeSelection(elem, helperMsg){
			if(elem.value == "Select here"){
				...
				return true;
                            ...
                            <div class="form-group">
                                  <label>Gender</label>
                                  <select class="form-control" name="gender" value="<?php echo $gender; ?>" id='gen' onchange='return formGen()'>
		...
		function isNumeric(elem, helperMsg){
			...
			if(elem.value.match(numericExpression)){
                ...
                <input type="text" class="form-control" placeholder="388001" name="zip" id='zip' value="<?php echo $zip; ?>"  onchange='return formZip()'>
                </div>
                  </div>
                  <div class="col-sm-3">
                    <div class="form-group">
...
$rows['uname']."/Images/".$rows['profile']."'class='img-responsive' width='150' height='150'><br>";
                              ...
                              echo"<br>";
                              echo '<input type="file" name="profile" value="" class="btn btn-info file" multiple >';
                          ...
                          <td >
                            <?php if($rows['resume']!="") {
                             echo" <iframe src = '../Faculty/".$rowdid['department']."/".$uname."/Resume/".$rows['resume']."'></iframe><br>";
             ...
             <div class="row">
                  <div class="col-sm-6">
                            ...
                            <td style="width:50px;"><input type="submit" class="btn btn-info" name="updateprofile" value="Submit"></td>
                  
                  
                    <td style="width:50px;">  <input type="reset" class="btn btn-danger" value="Reset">
                            </td>
                  </div>
                </div>
          </form>

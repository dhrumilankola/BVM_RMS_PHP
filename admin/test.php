        ...
        foreach ($_FILES['photos']['tmp_name'] as $key => $value) { 
            ...
            if(in_array(strtolower($file_ext),$allowed_types)) { 
                ...
                if(file_exists($photos)) { 
                    $photos = $upload_dir.time().$file_name; 
                      
                    if( move_uploaded_file($file_tmpname, $photos)) { 
...
//  $photos = $key.$_FILES['photos']['name'][$key];
//  $file_size =$_FILES['photos']['size'][$key];
//  $file_tmp =$_FILES['photos']['tmp_name'][$key];
//  $file_type=$_FILES['photos']['type'][$key];
//  // $desired_dir= $row['uname'].'/Conference/'.date('Y');
...
//  //       }
//     }
 // move_uploaded_file($file_tmp,"$desired_dir/".$photos);
    $sql= "INSERT INTO conference (name,attend_as,type, aid,date,decr, photos,  start_time,end_time)
VALUES ('$name','$as','$type','$session_aid','$date','$decr','$photos','$start_time','$end_time')";
               if(mysqli_query($link, $sql) )
                {
                   echo "<script>alert(Add Conference Successfully);</script>";
            ...
            <div class="card card-secondary">
              ...
              <div class="card-body">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-12">
                              <div class="col-sm-12">
                              <input type="submit" class="btn btn-info" name="addconference" value="submit">

...
//             }
  ...
  if($row3['user_type'] == "Admin"){
        $desired_dir='Papers/'.date('Y');
        if(empty($errors)==true){
            if(is_dir($desired_dir)==false){
                mkdir("$desired_dir", 0700);    // Create directory if it does not exist
            }
            if(is_dir("$desired_dir/".$file_name)==false){
                move_uploaded_file($file_tmp,'Papers/'.date('Y')."/".$file_name);
            }else{                  //rename the file if another one exist
                $new_dir='Papers/'.date('Y')."/".$file_name.time();
 ...
 $sqldid=mysqli_query($link,"select * from Department WHERE did IN (select did from user where aid=$row3[aid])");
    $rowdid=mysqli_fetch_array($sqldid);
     $desired_dir="../Faculty/".$rowdid['department']."/".$row3['uname']."/".date('Y')."/Papers/Documents";
        if(empty($errors)==true){
            ...
            if(is_dir("$desired_dir/".$file_name)==false){
                move_uploaded_file($file_tmp,"../Faculty/".$rowdid['department']."/".$row3['uname']."/".date('Y')."/Papers/Documents/".$file_name);
            }else{                  //rename the file if another one exist
                $new_dir="../Faculty/".$rowdid['department']."/".$row3['uname']."/".date('Y')."/Papers/Documents/".$file_name.time();
                 rename($file_tmp,$new_dir) ;       
            }
  ...
  if(empty($error)){
     $pdfs = implode("|", $imageData);
      $query= "UPDATE papers SET decr='$decr',pdf='$pdfs' WHERE paperid='$paperid'";
            ...
            <div class="card card-secondary">
                ...
                <!-- <div class="row"> -->
                 ...
                 <!--  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <input type="number" class="form-control" placeholder="Enter ..." name="total_que" >
                    </div>
                </div>
                   <div class="col-sm-4">
                    <div class="form-group">
                      <label>Total Marks</label>
                      <input type="number" class="form-control" placeholder="Enter ..." name="total_marks" >
                    </div>
                </div>
                <div class="col-sm-4">
                <div class="form-group">
              ...
              <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-12">
                    <div class="col-sm-12">
                    <input type="submit" class="btn btn-info" name="updatepaper" value="submit">

...
//             }
      ...
      elseif($row3['user_type'] == "Faculty"){  
        ...
        $desired_dir="../Faculty/".$rowdid['department']."/".$row3['uname']."/".date('Y')."/Seminar/Photos";
        if(empty($errors)==true){
            if(is_dir($desired_dir)==false){
                mkdir("$desired_dir", 0700);    // Create directory if it does not exist
            }
            if(is_dir("$desired_dir/".$file_name)==false){
                move_uploaded_file($file_tmp,"../Faculty/".$rowdid['department']."/".$row3['uname']."/".date('Y')."/Seminar/Photos/".$file_name);
            }else{                  //rename the file if another one exist
                $new_dir="../Faculty/".$rowdid['department']."/".$row3['uname']."/".date('Y')."/Seminar/Photos/".$file_name.time();
                 rename($file_tmp,$new_dir) ;       
            }
  ...
  if(empty($error)){
    ...
    $photos = implode("|", $imageData);
     $query= "UPDATE seminars SET name='$name',attend_as='$as',type='$type',date='$date',decr='$decr',photos='$photos',start_time='$start_time',end_time='$end_time' WHERE semid='$semid'";
        ...
        <div class="row">
            ...
            <div class="card card-secondary">
              <div class="card-header">
                <h3 class="card-title">Update Seminar</h3>
              </div> 
              <!-- /.card-header -->
              <div class="card-body">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-12">
                              <div class="col-sm-12">
                              <input type="submit" class="btn btn-info" name="updatesem" value="submit">
